...
try {
  ...
  const info = await page.evaluate(() => {
    const headlight = window.x3dom.canvases[0].doc._scene.getNavigationInfo()._vf.headlight;
    const markup = document.getElementById('x3dom-host').outerHTML.slice(0, 400);
    return { headlight, markup };
  });
