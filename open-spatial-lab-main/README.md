...
### Ubuntu / WSL setup (verified)

The smoke and spike suites were verified successfully from a local Ubuntu WSL environment. The same setup is recommended if you want to run the browser-based verification locally.

1. Install Ubuntu/WSL and open it.
2. Install the required Linux packages:

```bash
sudo apt-get update
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y bash curl lsof sed nodejs npm chromium-browser
```

3. Ensure the expected toolchain versions are present:
...
4. From the repository root, set the browser path for Puppeteer before running any browser-based verification:

```bash
export PUPPETEER_EXECUTABLE_PATH=/usr/bin/chromium-browser
```

5. Start the app and run the checks:
...
If you are running from WSL and the launcher reports stale port conflicts, use the Ubuntu-native recovery wrapper instead:

```bash
bash tools/ubuntu/run-with-recovery.sh
```

This helper resolves the repository from its own location, clears stale listeners on the Ubuntu-side ports, and relaunches the app from the repository root with the browser path preconfigured for Ubuntu/WSL.

If you prefer a persistent shell setup, add the browser path to your shell profile:
...
The verification scripts and launcher use the following environment variables when present:

- `PUPPETEER_EXECUTABLE_PATH` — path to a compatible browser executable for the browser-based verification scripts.
- `OSL_FRONTEND_PORT` — frontend port (default `8143`).
- `OSL_BACKEND_A_PORT` — backend A port (default `18151`).
- `OSL_BACKEND_B_PORT` — backend B port (default `18152`).
- `OSL_BACKEND_LOBBY_PORT` — lobby backend port (default `18153`).
- `OSL_BACKEND_AIRPORT_PORT` — airport backend port (default `18154`).
- `OSL_BASH_PATH` — optional override for the shell used by the verification scripts (useful on non-standard hosts).

Example:

```bash
export PUPPETEER_EXECUTABLE_PATH=/usr/bin/chromium-browser
export OSL_FRONTEND_PORT=8143
export OSL_BACKEND_A_PORT=18151
export OSL_BACKEND_B_PORT=18152
export OSL_BACKEND_LOBBY_PORT=18153
export OSL_BACKEND_AIRPORT_PORT=18154
```
